# -*- coding: utf-8 -*-
"""
CallGiant — Automated Calling System (Single-File Edition)
===========================================================
Five-tab tkinter GUI with SQLite persistence and real Twilio integration.

Run:  python call_giant.py
"""
...
def _app_dir() -> str:
    """Return a persistent writable directory for the database."""
...
# ╔═══════════════════════════════════════════════════════════════════╗
#  HELPERS — TTS & Phone Validation
# ╚═══════════════════════════════════════════════════════════════════╝

# Matches E.164 (+1234…) or local US 10-digit (with optional separators)
...
def is_valid_phone(number: str) -> bool:
    """Quick sanity check: at least 7 digits and matches pattern."""
    if not number:
        return False
...
class CallEngine:
    ...
    def pause_calling(self):
        if self.running and not self.paused:
            ...
            self.emit("log", "PAUSED — will pause after the current call finishes.")

    def resume_calling(self):
        if self.running and self.paused:
            self.paused = False
            self._pause_event.set()
            self.emit("log", "RESUMED — continuing calls.")
    ...
    def _call_loop(self):
        ...
        # Load settings
        account_sid   = get_setting("twilio_sid")
        auth_token    = get_setting("twilio_token")
        twilio_number = get_setting("twilio_number")

        try:
            delay = max(1, float(get_setting("call_delay", "5")))
        ...
        for i, lead in enumerate(leads):
            # Wait here while paused (unblocks instantly if not paused)
            ...
            if not is_valid_phone(phone):
                self.emit("log", f"  [SKIP] Invalid phone number: {phone}")
                add_call_log(
                    phone_number=phone, lead_name=name,
                    call_status="skipped: invalid phone",
                )
                continue
            ...
            # Delay between calls — check stop flag every 100 ms
...
def make_real_call(phone_number: str, lead_name: str = "") -> dict:
    ...
    connects. The hosted endpoint controls the call flow.
    ...
    if not is_valid_phone(phone_number):
        result["error"] = f"Invalid phone number format: {phone_number}"
        _log_call_result(phone_number, lead_name, result)
        return result
...
class CallGiantApp:
    ...
    def _import_leads(self):
        ...
        try:
            ...
            if leads:
                ...
                messagebox.showwarning("Import", "No valid leads found in the file.")
        except FileNotFoundError:
            messagebox.showerror("File Not Found", f"Could not open file:\n{path}")
        except PermissionError:
            messagebox.showerror("Permission Error", f"Cannot read file (it may be open elsewhere):\n{path}")
        except Exception as e:
    ...
    def _ask_user_for_column(self, columns, title, prompt_text):
        """Show a modal dialog listing all column names.  Returns selected or None."""
        result = [None]
        ...
        def on_select(event=None):
            ...
            if sel:
                hint_var.set(f'Selected: "{columns[sel[0]]}"')
    ...
    def _build_message_tab(self):
        ...
        bar = ttk.Frame(tab)
        bar.pack(fill=tk.X)
        ttk.Button(bar, text="Test TTS (speakers)", command=self._test_tts).pack(side=tk.LEFT)
    ...
    def _build_settings_tab(self):
        ...
        for i, (key, label, ph, secret) in enumerate(fields):
            ...
            var = tk.StringVar()
            ttk.Entry(grp1, textvariable=var, width=52, show="*" if secret else "").grid(
        ...
        # ── Quick-Start Guide ──────────────────────────────────────
        grp4 = ttk.LabelFrame(tab, text="Quick-Start Guide — How to Set Up Twilio Calls", padding=12)
        grp4.pack(fill=tk.X, pady=(0, 10))
        ttk.Label(
            grp4,
            text=(
                "1.  Sign up at twilio.com and grab your Account SID + Auth Token.\n"
                "2.  Buy a Twilio phone number (the number that will make outbound calls).\n"
                "3.  Enter those credentials and the Twilio number above.\n"
                "4.  Set the Agent Number — this is where calls transfer when the lead presses 1.\n"
                "\n"
                "5.  Deploy webhook.py on your server so Twilio can reach it at:\n"
                "       https://yourdomain.com/voice  and  /dtmf\n"
                "6.  Set the BASE_URL environment variable to your domain\n"
                "       (e.g. BASE_URL=https://yourdomain.com).\n"
                "\n"
                "7.  Go to the Message tab and write your TTS script.\n"
                "8.  Import leads, then hit Start Calling — you're live!"
            ),
            style="Dim.TLabel",
    ...
    def _setup_auto_save(self):
        for key, var in self.setting_vars.items():
            var.trace_add("write", lambda *_a, k=key, v=var: save_setting(k, v.get()))
    ...
    def _build_control_tab(self):
        ...
        self.pause_btn = tk.Button(
            ...
            font=("Segoe UI", 18, "bold"), bg=self.ACCENT, fg="white",
    ...
    def _start_calling(self):
        ...
        for key, var in self.setting_vars.items():
            save_setting(key, var.get())
        save_setting("tts_message", self.message_text.get("1.0", tk.END).strip())

        self.start_btn.config(state=tk.DISABLED)
        self.stop_btn.config(state=tk.NORMAL)
        self.pause_btn.config(state=tk.NORMAL, text="\u23f8  PAUSE", bg=self.ACCENT)
    ...
    def _stop_calling(self):
        ...
        self.pause_btn.config(state=tk.DISABLED, text="\u23f8  PAUSE", bg=self.ACCENT)
    ...
    def _toggle_pause(self):
        ...
        if self.engine.paused:
            ...
            self.pause_btn.config(text="\u23f8  PAUSE", bg=self.ACCENT)
            self.progress_var.set("Resumed")
            self.status_var.set("Calls resumed")
        else:
            self.engine.pause_calling()
            self.pause_btn.config(text="\u25b6  RESUME", bg="#f39c12")
            self.progress_var.set("Paused — waiting for current call to finish")
    ...
    def _poll_engine(self):
        """Drain engine message queue and update UI (runs every 100 ms)."""
        try:
            while True:
